...
class DotenvTest extends TestCase
{
    ...
    public function testLoad()
    {
        ...
        $this->assertInternalType('int', $_SERVER['ZERO_LITERAL']);
        $this->assertEquals(0, $_SERVER['ZERO_LITERAL']);

        $this->assertInternalType('string', $_SERVER['ZERO_QUOTED']);
        $this->assertEquals('0', $_SERVER['ZERO_QUOTED']);

        $this->assertInternalType('int', $_SERVER['NUMBER_LITERAL']);
        $this->assertEquals(1111, $_SERVER['NUMBER_LITERAL']);

        $this->assertInternalType('string', $_SERVER['NUMBER_QUOTED']);
    ...
    public function testIncompatibleProcessors()
    {
        ...
        $this->assertProcessors(
            ['\Srdorado\Env\Processor\QuotedProcessor', null],
            ['\Srdorado\Env\Processor\QuotedProcessor']
        );
    }
     ...
     * @runInSeparateProcess
     */
    private function assertProcessors($processorsToUse = null, $expectedProcessors = [])
    {
    ...
    private function getProtectedProperty($object)
    {
    ...
    {
        (new DotEnv($this->env('.env.boolean'), ['\Srdorado\Env\Processor\BooleanProcessor']))->load();

        $this->assertEquals(false, $_ENV['FALSE1']);
        $this->assertEquals(false, $_ENV['FALSE2']);
        $this->assertEquals(false, $_ENV['FALSE3']);
        // Since we don't have the QuotedProcessor::class this will be the result
    ...
    {
        (new DotEnv($this->env('.env.quotes'), ['\Srdorado\Env\Processor\QuotedProcessor']))->load();
    ...
    {
        (new DotEnv($this->env('.env.number'), ['\Srdorado\Env\Processor\NumberProcessor']))->load();
